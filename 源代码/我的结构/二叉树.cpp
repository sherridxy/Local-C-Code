...
public:
	...
	void InOrderInMirror(TreeNode<T> *subTree,void(*visit)(TreeNode<T> *p));//镜像中序遍历
	friend TreeNode<char> * operator >>(istream &in,BinTree<char> & B);
	friend void operator<<(ostream &out,BinTree<char> &B);
};


TreeNode<char> *  operator >> (istream &in,BinTree<char> & B) //已声明为模板树的友元函数，可以访问其任意实例
{
...
template <class T>
void BinTree<T>::InOrderInMirror(TreeNode<T> *subTree,void(*visit)(TreeNode<T> *p))
{
    if (subTree==NULL) return;
    InOrderInMirror(subTree->rightChild,visit);
    visit (subTree);
    InOrderInMirror(subTree->leftChild,visit);
}

void test (TreeNode<char> *p)
{
    cout<<p->data<<' ';
}
...
void Show(TreeNode<char> *p)
{
    int i;
    BinTree<char> temp;
    int num=temp.TellDep(p);
    for (i=1;i<num;i++) cout<<' ';
    cout<<p->data<<endl;
}

void operator<<(ostream &out,BinTree<char> &B)
{
    B.InOrderInMirror(B.root,Show);
}
